                              ...
                              GParamSpec * pspec)
{
  ...
  switch (property_id)
    {
    case PROP_THRD:
      g_value_set_pointer (value, self->thrd);
      break;
    case PROP_NAME:
    ...
    case PROP_ACTIVE:
      ...
      break;
    case PROP_CURRENT:
      g_value_set_current (value, self->current);
      break;
    default:
...
gdn_thread_info_class_init (GdnThreadInfoClass *klass)
{
  ...
  properties[PROP_THRD] =
      g_param_spec_string ("thread", "thread", "thread pointer", NULL, G_PARAM_READABLE);
...
static void
gdn_thread_info_init (GdnThreadInfo *self)
{
}
...
{
  const static SCM format_string = scm_from_utf8_string ("~A");
  ...
  ptr = SCM_PACK_POINTER (thread);
  name = scm_simple_format (SCM_BOOL_F, format_string,
                            scm_list_1 (thread));
  active = scm_is_false (scm_thread_exited_p (thread));
  current = scm_is_true (scm_eqv_p (thread, current_thread));
  self->thrd = SCM_POINTER_PACK (thread);
...
gdn_thread_info_store_update (GListStore *store)
{
  ...
  for (i = 0; i < scm_c_vector_length (all_threads); i++)
    {
      SCM            thrd;
      GdnThreadInfo *info;
